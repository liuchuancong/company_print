...
class WebDavPage extends GetView<WebDavController> {
      ...
      actions: [
        PopupMenuButton<int>(
          ...
          itemBuilder: (BuildContext context) => [
            const PopupMenuItem(
              value: 1,
              child: MenuListTile(leading: Icon(Icons.refresh), text: '刷新'),
            ),
            const PopupMenuItem(
              value: 2,
              child: MenuListTile(leading: Icon(Icons.menu), text: '打开配置列表'),
            ),
          ],
        ),
      ],
  ...
  List<Widget> _buildBreadcrumbs() {
    ...
    for (String part in controller.breadcrumbParts) {
      buttons.add(const Padding(padding: EdgeInsets.symmetric(horizontal: 4), child: Icon(Icons.navigate_next)));
