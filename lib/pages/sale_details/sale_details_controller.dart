...
class SaleDetailsController extends GetxController {
  ...
  void copyTextToClipboard() async {
    String text = '总价: ${Utils.getString(getTotalOrderPrice.value)}元';
    final List<OrderItem> products = await database.orderItemsDao.getAllOrderItemsByOrderId(orderId);

    for (var i = 0; i < products.length; i++) {
      final product = products[i];
      text +=
          '\n${Utils.getString(product.itemName)} 数量: ${Utils.getString(product.actualQuantity)} 单价: ${Utils.concatenation(product.actualPrice, product.actualUnit)} 总价: ${getOrderCalculationType(product.totalPrice)} 备注: ${Utils.getString(product.itemShortName)}';
    }
